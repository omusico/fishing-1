...
class Bbs extends CI_Controller {
	function add() {
		...
		json_decode($data['pic']) OR errInput();
		$data['time']=time();
		$data['uid']=UID;
	...
	function getList() {
		...
		$select='id,(SELECT name FROM user WHERE user.id=uid) authorName,title,time';
	...
	function item() {
		...
		$data=$this->db->find('bbs', $id,'id','id,title,content,pic,time,(SELECT avatar FROM user WHERE user.id=uid) authorAvatar,(SELECT name FROM user WHERE user.id=uid) authorName');
	...
	function commentList() {
		...
		$page=(int)$this->input->post('page',FALSE,0);
		$count=(int)$this->input->post('count',FALSE,10);
		$data=$this->db->select('id,content,time,(SELECT avatar FROM user WHERE user.id=uid) authorAvatar,(SELECT name FROM user WHERE user.id=uid) authorName')
			->where('bid',$id)->limit($count,$count*$page)->order_by('id','desc')
			->get('bcomment')->result_array();
		ajax(0,'',$data);
	}
}